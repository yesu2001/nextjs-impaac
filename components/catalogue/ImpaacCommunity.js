...
export default function ImpaacCommunity({ image }) {
  const isMobile = useResponsive("down", "sm");
  return (
    <Container maxWidth="lg">
      <Grid container sx={{ py: 10, px: { md: 5 } }} spacing={3}>
        <Grid item xs={12} md={6}>
          <CatalogueImage image={image} alt="contact" />
        </Grid>
        ...
        <Grid item xs={12} sm={12} md={6} lg={6}>
          ...
          <Stack spacing={3} mt={4} sx={{ mx: { sm: 5, md: 0, xs: 5 } }}>
            <ReasonBox>
              <Box flex={0.2}>
                <Image alt={msg} src={msg} height={isMobile ? 40 : 60} />
              </Box>
            ...
            <ReasonBox>
              <Box flex={0.2}>
                <Image alt={call} src={call} height={isMobile ? 40 : 60} />
              </Box>
